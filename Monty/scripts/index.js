...
function switchDoors(bool, sadcat) {
  ...
  if (version === 1 || version === 2 || version === 3) {
    document.querySelector(`#door${internalOption}2`).style.backgroundImage = `url(./assets/sadcat${ !sadcat}.png)`;
    document.querySelector(`#door${internalChoice}2`).style.backgroundImage = 'url(./assets/chocolate.avif)';
    if (switchDoor === true) {
      switchLosses++;
      recordGameData(true, false);
      document.querySelector('#message',).innerHTML = `<p class="lose">Oh no, the chocolate was behind door #${internalChoice}. The kittens are sad :(</p>${resetP}`;
    } else if (switchDoor === false) {
      stayWins++;
      recordGameData(false, true);
      document.querySelector('#message',).innerHTML = `<p class="win">You win! Try again?.</p>${resetP}`;
    }
  } else {
    document.querySelector(`#door${internalOption}2`).style.backgroundImage = 'url(./assets/chocolate.avif)';
    document.querySelector(`#door${internalChoice}2`).style.backgroundImage = `url(./assets/sadcat${ !sadcat}.png)`;
    ...
    if (switchDoor === true) {
      ...
      document.querySelector('#message',).innerHTML = `<p class="lose">Oh no, the chocolate was behind door #${internalOption}. You made a kitten cry :(</p>${resetP}`;
    }
  }
...
function letsGo(userChoice, userWinner, userHint, option, sadcat) {
  internalChoice = userChoice;
  internalWinner = userWinner;
  internalHint = userHint;
  internalOption = option;
  internalSadcat = sadcat;
  rotateDoors();
  document.getElementById(`hoversheet${internalHint}2`).disabled = true;
  document.querySelector('#message',).innerHTML = `There's a sad cat behind door #${internalHint}, so the chocolate is either behind door #${internalChoice} (your original choice) or door #${internalOption}.`;
  document.querySelector(`#door${internalChoice}2`).setAttribute('onclick', 'switchDoors(false, internalSadcat)');
  document.querySelector(`#door${internalOption}2`).setAttribute('onclick', 'switchDoors(true, internalSadcat)');
  ...
  document.querySelector(`#door${internalHint}2`).style.backgroundImage = `url(./assets/sadcat${internalSadcat}.png)`;
...
function play(chosenDoor) {
  // SET SADCAT RANDOMNESS BOOLEAN
  const sadcat = Math.random() < 0.5;
  ...
  if (choice === 1 && winner === 1) {
    ...
    if (hint === 0) {
      // hint=2
      letsGo(1, 1, 2, 3, sadcat);
    } else {
      // hint=3
      letsGo(1, 1, 3, 2, sadcat);
    }
  }
  ...
  if (choice === 2 && winner === 2) {
    ...
    if (hint === 0) {
      // hint=1
      letsGo(2, 2, 1, 3, sadcat);
    } else {
      // hint=3
      letsGo(2, 2, 3, 1, sadcat);
    }
  }
